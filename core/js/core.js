...
(function($){
    ...
    core.ajax = (function(){
        ...
        function _ws(url,request,callback){
            ...
            $.ajax({
                    ...
                    success: function (response) {
                        ...
                        Core.util.console("*********************");
                        resp = {'status':{'nerror':0,'serror':'Ajax operation success'},'response':response};
                        callback(resp);
                    },
                    error: function (response) {
                        var isCompleted = false;

                        if(Core.util.isValid(response)){
                            if(Core.util.isValid(response.status)){
                        ...
                        Core.util.console("*********************");
                        if(isCompleted){
                            Core.util.console("Edited Ajax Success");
                            resp = {'status':{'nerror':0,'serror':'Edited ajax operation success'},'response':response.responseText};
                        }else{
                            Core.util.console("Ajax Error");
                            resp = {'status':{'nerror':-1,'serror':'Ajax operation failed'},'response':undefined};
                        }
        ...
        function _download(url,request,callback){
            $.ajax({
                crossDomain: true,
                'url': url,
                'type': "POST",
                ...
                success: function (response) {
                    Core.util.console("*********************");
                    Core.util.console("Ajax Download Success");
                    Core.util.console("*********************");
                    resp = {'status':{'nerror':0,'serror':'Ajax operation success'},'response':response};
                    callback(resp);
                },
                error: function (response) {
                    ...
                    Core.util.console("*********************");
                    if(isCompleted){
                        Core.util.console("Edited Ajax Download Success");
                        resp = {'status':{'nerror':0,'serror':'Edited ajax operation success'},'response':response.responseText};
                    }else{
                        Core.util.console("Ajax Download Error");
                        resp = {'status':{'nerror':-1,'serror':'Ajax operation failed'},'response':undefined};
                    }
        ...
        return{
            ...
            download: function(url,request,callback){
                _download(url,request,callback);
            }
        }
    })();
    ...
    core.modal = (function(){
        ...
        function _wait(message){
            ...
            if(typeof message != "undefined"){
                ...
                function parpadear(){
                    p.fadeIn(1000).delay(100).fadeOut(1000, parpadear);
                }
                parpadear();
            }
            modal.show();
